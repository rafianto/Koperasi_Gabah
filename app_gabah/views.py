...
from django.utils import timezone # <--- PASTIKAN INI ADA
...
def sp_lunasi(request, pk):
    ...
    obj.status = 'lunas'
    obj.save()
    messages.success(request, f'Pinjaman {obj.anggota.nama} sebesar Rp {obj.jumlah_rp:,.0f} berhasil dilunasi!')
    return redirect('sp_list')
...
def cashflow_view(request):
    ...
    # Ambil parameter dari URL (jika user memfilter tanggal)
    ...
    # Panggil Utility Class Cashflow
    report = CashflowReport(start_date, end_date)
    context = report.get_summary()

    # Tambahkan data tanggal untuk form filter
    context['start_date'] = start_date.strftime('%Y-%m-%d')
    context['end_date'] = end_date.strftime('%Y-%m-%d')

    # PENTING: Sesuaikan path template dengan struktur folder Anda
    return render(request, 'app_gabah/cashflow_template.html', context)
